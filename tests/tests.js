...
async function runKeyringTest() {
    ...
    expect(
        initialAcc[0]["0xcc597bd2e7eda5094d6aa462523b629a502db6cc71a6ae0e9b158d9e42c6c462"], "15cwMLiH57HvrqBfMYpt5AgGrb5SAUKx7XQUcHnBSs2DAsGt");
...
async function runAccountTest() {
    ...
    const addr2 = "23EH25JzLSfStdUgsojcdwetgsUUCqoZcraC3fyb2vNWTUvS";
    const info = await account.getAccountIndex(api, [addr2]);
    expect(info[0].accountId.toString(), addr2);
    expect(info[0].identity.display, "Setheum Labs");
    expect(info[0].identity.web, "https://setheum.xyz");
